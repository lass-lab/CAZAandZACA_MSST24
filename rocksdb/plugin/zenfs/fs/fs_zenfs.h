...
class ZenFS : public FileSystemWrapper {
  ...
  uint64_t GetAsyncStructure(io_uring** read_ring,io_context_t** write_ioctx){
    ...
    if(io_uring_queue_.empty()){
      ...
      err= io_uring_queue_init(1000*one_zc_reclaimed_zone_n_, ret_read_ring, flags);
      if(err){
    ...
    if(io_ctx_queue_.empty()){
      ...
      err=io_queue_init(1000*one_zc_reclaimed_zone_n_,ret_write_ioctx);
      if(err){
...
// loop:
    ...
    if(zbd_->GetZCRunning() ){
      ...
      while(CalculateFreePercent()<=zbd_->GetZoneCleaningKickingPoint())
