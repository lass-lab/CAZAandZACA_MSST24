...
uint64_t ZenFS::GetMaxInvalidateCompactionScore(std::vector<uint64_t>& file_candidates,uint64_t * candidate_size) {
  // return file_candidates.size();
  uint64_t ret = 100* zbd_->GetMaxSameZoneScore(file_candidates);
  return ret;
  (void)(candidate_size);
