...
DEFINE_int32(writer_threads,1,"Number of writer thread.");
...
DEFINE_uint64(pca_selection,0,"pca_selection");
...
class Duration {
 public:
  ...
  uint64_t GetOPS(void) {return ops_;}
 private:
...
//////////////////////////////////////////////////////////////////
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    int writer_threads = FLAGS_writer_threads;
    for (int i = 0; i < n; i++) {
#ifdef NUMA
      if (FLAGS_enable_numa) {
      ...
      if(i<writer_threads){
        arg[i].thread->writer= true;
      }else{
        arg[i].thread->writer=false;
      }
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    printf("options.compaction_pri %d\n",options.compaction_pri);
...
// optimization, so skip the check of the max level.
  ...
  void ReadWhileWriting(ThreadState* thread) {
    if(thread->writer){
      printf("writer thread for %d\n",thread->tid );
      BGWriter(thread, kWrite);
    }else{
      printf("reader thread for %d\n",thread->tid );
      ReadRandom(thread);
    }
  }
  ...
  void BGWriter(ThreadState* thread, enum OperationType write_merge) {
    ...
    while (true) {
      DB* db = SelectDB(thread);
      // printf("DB POINTER AT WRITER %p\n",db);
      // printf("WRite@@@@2  thread->tid %d\n",thread->tid);
      {
  ...
  void ReadRandomWriteRandom(ThreadState* thread) {
    ...
    printf("READRANDOMMWRITERANDOM ALL DONE\n");
    char msg[100];
