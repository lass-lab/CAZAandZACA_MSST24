...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  other_options.push_back(immutable_db_options_.pca_selection);
...
std::vector<int> DBImpl::NumLevelsFiles(void) {
  ...
  auto vstorage=versions_->GetColumnFamilySet()->GetDefault()->current()->storage_info();
  // versions_
  // return versions_->GetColumnFamilySet()->GetDefault()->current()->storage_info()->NumLevelFiles(level);
  for(int i = 0; i< 6;i++){
...
double DBImpl::ReCalculateCompactionScore(int level){
  ...
  auto moptions = versions_->GetColumnFamilySet()->GetDefault()->GetCurrentMutableCFOptions();
  return vstorage->ReCalculateCompactionScore((*ioptions),
                (*moptions),level);
  // double ret = 0.0;
  // int level;
...
uint64_t DB::MostSmallDownwardAdjacentFile(Slice& , Slice& , int ){
  return 0;
}
